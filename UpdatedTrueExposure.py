...
True Exposure is auto-calculated as:
    (Weight of ETF in Fund) x (Weight of Holding in ETF)
...
# ======================================================================================
STD_COLS = ["SL_NO", "ETF", "ETF_WEIGHT_IN_FUND", "HOLDING", "HOLDING_WEIGHT_IN_ETF"]
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["ETF_WEIGHT_IN_FUND", "HOLDING_WEIGHT_IN_ETF"]:
...
def compute_exposure(df: pd.DataFrame) -> pd.DataFrame:
    """Auto-calculate True Exposure = (Weight of ETF in Fund) x (Weight of Holding in ETF).
    Auto-detects whether the two weight columns are fractions (0-1) or percentages (0-100)
    so both are placed on a consistent 0-100 scale before multiplying."""
    df = df.copy()
    ...
    df["EXPOSURE"] = (df["ETF_WEIGHT_IN_FUND"] * df["HOLDING_WEIGHT_IN_ETF"]) / 100.0
    return df
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "ETF'S HOLDING<br>WEIGHT IN ETF"
        "</div>"
        "<div style='font-size:11px; color:#999; margin-top:6px;'>"
        "True Exposure is calculated automatically as Weight of ETF in Fund &times; Weight in ETF."
        "</div>",
        unsafe_allow_html=True,
    )
...
if st.session_state.master_df is None or st.session_state.master_df.empty:
    st.info("Upload a CSV or Excel file from the sidebar to begin. Columns can be in any order / spacing — "
            ...
            "ETF'S HOLDING, WEIGHT IN ETF. True Exposure is calculated automatically.")
    st.stop()
